...
(function (global, factory) {
  ...
  var pointerout = HAS_POINTER_EVENTS
    ? "pointerout"
  ...
  var taPropPrefixed = getPrefixedPropName(
  ...
  Dragger._bindListeners = function () {
    window.addEventListener(
      Dragger._inputEvents.move,
      Dragger._onMove,
      listenerOptions
    );
    window.addEventListener(
      Dragger._inputEvents.end,
      Dragger._onEnd,
      listenerOptions
    );
    if (Dragger._inputEvents.cancel) {
      window.addEventListener(
        Dragger._inputEvents.cancel,
        Dragger._onCancel,
        listenerOptions
      );
    }
  ...
  Dragger.prototype._createEvent = function (type, e) {
    ...
    return {
      ...
      deltaTime:
        type === Dragger._emitterEvents.start ? 0 : this.getDeltaTime(),
      isFirst: type === Dragger._emitterEvents.start,
      isFinal:
        type === Dragger._emitterEvents.end ||
        type === Dragger._emitterEvents.cancel,
  ...
  function getContentRect(element, result) {
    ...
    if (element === window) {
      ...
      var borderLeft =
        element.clientLeft || getStyleAsFloat(element, "border-left-width");
      var borderTop =
        element.clientTop || getStyleAsFloat(element, "border-top-width");
  ...
  function computeThreshold(threshold, safeZone, itemSize, targetSize) {
    ...
    THRESHOLD_DATA.offset =
      Math.max(
        0,
        itemSize + THRESHOLD_DATA.value * 2 + targetSize * safeZone - targetSize
      ) / 2;
  ...
  ScrollRequest.prototype.computeNextScrollValue = function (deltaTime) {
    ...
    var nextValue =
      FORWARD & this.direction ? this.value + delta : this.value - delta;
  ...
  AutoScroller.prototype._checkItemOverlap = function (item, checkX, checkY) {
    ...
    var targets = isFunction(settings.targets)
      ? settings.targets(item)
      : settings.targets;
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (
        (!testAxisX || testPriority < xPriority) &&
        (!testAxisY || testPriority < yPriority)
      ) {
        continue;
      }
      ...
      if (
        ...
        if (dragDirectionX === RIGHT) {
          ...
          if (
            testDistance <= testThreshold.value &&
            getScrollLeft(testElement) < testMaxScrollX
          ) {
            testDirection = RIGHT;
          }
      ...
      if (
        ...
        if (dragDirectionY === DOWN) {
          testDistance =
            testRect.bottom + testThreshold.offset - itemRect.bottom;
          if (
            testDistance <= testThreshold.value &&
            getScrollTop(testElement) < testMaxScrollY
          ) {
            testDirection = DOWN;
          }
  ...
  AutoScroller.prototype._updateScrollRequest = function (scrollRequest) {
    ...
    for (var i = 0; i < targetCount; i++) {
      ...
      testScroll = testIsAxisX
        ? getScrollLeft(testElement)
        : getScrollTop(testElement);
      hasReachedEnd =
        FORWARD & scrollRequest.direction
  ...
  AutoScroller.prototype._updateRequests = function () {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      needsCheck =
        checkTime > 0 &&
        this._tickTime - checkTime > this._overlapCheckInterval;
  ...
  AutoScroller.prototype._updateDragDirection = function (item) {
    ...
    if (dragPositions.length) {
      ...
      dragDirections[0] =
        x1 > x2 ? RIGHT : x1 < x2 ? LEFT : dragDirections[0] || 0;
      dragDirections[1] =
        y1 > y2 ? DOWN : y1 < y2 ? UP : dragDirections[1] || 0;
    }
    dragPositions[0] = x1;
    dragPositions[1] = y1;
  };
  ...
  var transformProp =
    getPrefixedPropName(document.documentElement.style, "transform") ||
    "transform";
  ...
  function ItemDrag(item) {
    ...
    this._handle =
      (settings.dragHandle && element.querySelector(settings.dragHandle)) ||
      element;
  ...
  ItemDrag.defaultStartPredicate = function (item, event, options) {
    ...
    if (!predicate) {
      var config =
        options || drag._getGrid()._settings.dragStartPredicate || {};
    ...
    return function (item, options) {
      ...
      var sortThreshold =
        options && typeof options.threshold === "number"
          ...
          : 50;
      var sortAction =
        options && options.action === ACTION_SWAP ? ACTION_SWAP : ACTION_MOVE;
      var migrateAction =
        options && options.migrateAction === ACTION_SWAP
          ? ACTION_SWAP
          : ACTION_MOVE;
      ...
      sortThreshold = Math.min(
        Math.max(sortThreshold, minThreshold),
        maxThreshold
      );
  ...
  ItemDrag.prototype._unbindScrollListeners = function () {
    ...
    for (i = 0; i < scrollers.length; i++) {
      scrollers[i].removeEventListener(
        "scroll",
        this._onScroll,
        SCROLL_LISTENER_OPTIONS
      );
  ...
  ItemDrag.prototype._checkHeuristics = function (x, y) {
    ...
    if (Math.abs(diffX) > minDist || Math.abs(diffY) > minDist) {
      ...
      if (canCheckBounceBack) {
        ...
        var deltaAngle = Math.atan2(
          Math.sin(angle - prevAngle),
          Math.cos(angle - prevAngle)
        );
  ...
  ItemDrag.prototype._handleSort = function () {
    ...
    if (
      !settings.dragSort ||
      (!settings.dragAutoScroll.sortDuringScroll &&
        ItemDrag.autoScroller.isItemScrolling(this._item))
    ) {
  ...
  ItemDrag.prototype._prepareMove = function () {
    ...
    var prevEvent =
      this._dragPrevMoveEvent || this._dragStartEvent || nextEvent;
  ...
  var HAS_NATIVE_WEB_ANIMATIONS = !!(
    Element && isNative(Element.prototype.animate)
  );
  ...
  ItemMigrate.prototype.start = function (targetGrid, position, container) {
    ...
    // Update visibility class.
    currentVisClass = isVisible
      ? settings.itemVisibleClass
      : settings.itemHiddenClass;
    nextVisClass = isVisible
      ? targetSettings.itemVisibleClass
      : targetSettings.itemHiddenClass;
    if (currentVisClass !== nextVisClass) {
      removeClass(element, currentVisClass);
  ...
  function ItemVisibility(item) {
    ...
    addClass(
      element,
      isActive ? settings.itemVisibleClass : settings.itemHiddenClass
    );
   ...
   * @param {Function} [onFinish]
   */
  ItemVisibility.prototype._startAnimation = function (
    toVisible,
    instant,
    onFinish
  ) {
  ...
  function createPackerProcessor(isWorker) {
    ...
    PackerProcessor.prototype.computeNextSlot = function (
      ...
      for (i = 0; i < currentRects.length; i++) {
        ...
        if (
          slot.width <= rect.width + EPS &&
          slot.height <= rect.height + EPS
        ) {
          slot.left = rect.left;
          slot.top = rect.top;
          break;
      ...
      if (nextRects.length > 1) {
        this.purgeRects(nextRects).sort(
          horizontal ? this.sortRectsLeftTop : this.sortRectsTopLeft
        );
      }
    ...
    PackerProcessor.prototype.splitRect = (function () {
      ...
      return function (rect, hole) {
        ...
        if (
          ...
          shards.push(
            this.addRect(rect.left, rect.top, rect.width, rect.height)
          );
          return shards;
        }
        ...
        if (width >= MIN_SLOT_SIZE) {
          shards.push(
            this.addRect(hole.left + hole.width, rect.top, width, rect.height)
          );
        }
        ...
        if (height >= MIN_SLOT_SIZE) {
          shards.push(
            this.addRect(rect.left, hole.top + hole.height, rect.width, height)
          );
        }
  ...
  function Packer(numWorkers, options) {
    ...
    if (numWorkers && isWorkerProcessorsSupported()) {
      try {
        this._workers = createWorkerProcessors(
          numWorkers,
  ...
  Packer.prototype._finalizeLayout = function (layout) {
    ...
    if (isHorizontal) {
      ...
      layout.styles.height =
        (isBorderBox
          ? layout.height + grid._borderTop + grid._borderBottom
          : layout.height) + "px";
    }
   ...
   * @returns {?Function}
   */
  Packer.prototype.createLayout = function (
    grid,
    layoutId,
    items,
    width,
  ...
  function isPlainObject(val) {
    return (
      typeof val === objectType && toString.call(val) === objectToStringType
    );
  }
  ...
  Grid.prototype.getItem = function (target) {
    ...
    if (typeof target === NUMBER_TYPE) {
      return (
        this._items[target > -1 ? target : this._items.length + target] || null
      );
    }
  ...
  Grid.prototype.add = function (elements, options) {
    ...
    if (needsLayout && layout) {
      this.layout(
        layout === INSTANT_LAYOUT,
        isFunction(layout) ? layout : undefined
      );
    }
  ...
  Grid.prototype.send = function (item, targetGrid, position, options) {
    ...
    var layoutSender = opts.layoutSender
      ? opts.layoutSender
      : opts.layoutSender === undefined;
  ...
  Grid.prototype._onLayoutDataReceived = (function () {
    ...
    return function (layout) {
      if (
        this._isDestroyed ||
        !this._nextLayoutData ||
        this._nextLayoutData.id !== layout.id
      )
        return;
  ...
  Grid.prototype._setItemsVisibility = function (items, toVisible, options) {
    ...
    function triggerVisibilityChange() {
      ...
      for (i = 0; i < targetItems.length; i++) {
        ...
        targetItems[i]._visibility[method](
          isInstant,
          function (interrupted, item) {
